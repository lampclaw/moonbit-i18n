...
| 浏览器 runtime | 锁定 Playwright `1.62.1` 提供的 Chromium、Firefox、WebKit | 生成 facade、上游 MF2 suite、differential case 与应用场景 |
...
- 每个 `0.x` 依赖都应精确固定。路线图 minor 可以有意改变公共或生成契约；其
  changelog 与迁移诊断定义边界。已批准的 `0.2.x–0.9.x` 序列直接发布 stable，
...
- 在 `1.0` 前，不可避免的公共破坏通常要在更早的 minor version 提供诊断与迁移路径。
  `0.9.0` 中省略 `messageProfile` 是 error `I18N1003`；稳定 authoring 显式选择
  `unicode-mf2-ldml48.2-js-v2`。Legacy standards v1 作为带 warning `I18N1004` 的迁移
  桥梁保留，Draft date/time 必须使用独立版本化的 experimental profile。私有
  `:lampclaw:datetime` 只保留在兼容 profile，并报告 `I18N3003`。
- 公开 MoonBit interface hash、生成 template、CLI surface、wire version、profile 与
  diagnostic 记录在 `docs/contracts/1.0-candidate.json`；CI 会把未经审查的漂移视为发布失败。
