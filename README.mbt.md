...
MoonBit. `0.1.0-rc.3` ships one module containing the runtime, generator, and a
...
`0.1.0-rc.3` is an engineering-ready release candidate for Web/JavaScript
...
`moon add --bin lampclaw/i18n@0.1.0-rc.3` is an optional project-local binary
dependency, not the primary workflow. A global command can alternatively be installed with
`moon install lampclaw/i18n/cmd/i18n@0.1.0-rc.3`; it is named `moon-i18n`.
